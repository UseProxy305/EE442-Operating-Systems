...
//I choose scheduler algorithm as priority with ID
//Smallest ID means it has highest priority
...
//Selection Algorithm
//IDPrio it will choose the smallest index number thread with a ready position
int selectionIDPrio(void){
	int id=-1;
	for(int i=0;i<NUM_THREAD;i++){
		if(berkay[i+1]->state==0){
			id=i+1;
			break;		
		}
	}
